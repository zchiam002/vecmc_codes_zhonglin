...
##Version 2: storage capabilities

def extract_and_process_values_v2 (ret_dataframes, utility_list, storage_list, process_list, bilinear_pieces, solver_choice, parallel_thread_num, affected_list):
    ...
    ##storage_list --- the list of all storages
    ...
    if solver_choice == 'gurobi':
        obj_value, results, results_y = extract_gurobi_results(utility_list, storage_list, process_list, bilinear_pieces, parallel_thread_num)
        
    elif solver_choice == 'glpk':
        obj_value, results, results_y = extract_glpk_results (ret_dataframes, utility_list, storage_list, process_list, bilinear_pieces, parallel_thread_num, affected_list)
...
def extract_glpk_results (ret_dataframes, utility_list, storage_list, process_list, bilinear_pieces, parallel_thread_num, affected_list):
    ...
    result_file = 'C:\\Optimization_zlc\\slave_convex_handlers\\solver_lp_format_holder\\' + r'\t' + 'hread_' + str(parallel_thread_num) + '\\out.txt'
    ...
    data_needed['storagelist_bilinear'] = ret_dataframes['storagelist_bilinear']
    ...
    data_needed['storagelist_linear'] = ret_dataframes['storagelist_linear']     
    ...
    final_dataframe, final_dataframe_y = convert_back_to_original (result_dataframe, utility_list, storage_list, process_list, bilinear_pieces)
        
    return obj_value, final_dataframe, final_dataframe_y

##This function extracts the results from the gurobi output file 
def extract_gurobi_results (utility_list, storage_list, process_list, bilinear_pieces, parallel_thread_num):
    ...
    ##Result file location 
    result_file = 'C:\\Optimization_zlc\\slave_convex_handlers\\solver_lp_format_holder\\' + r'\t' + 'hread_' + str(parallel_thread_num) + '\\out.sol'
    
    ##Extracting the objective function value 
    with open(result_file) as fo:
        for rec in fo:
            if 'Objective value' in rec:
                obj = rec.split(' ')
                break
        obj_value = float(obj[4])    
    ...
    with open(result_file) as fo:
        for rec in fo:
            if '#' not in rec:
                variable_values = rec.split(' ')
                
                temp_values = [variable_values[0], float(variable_values[1])]
                temp_df = pd.DataFrame(data = [temp_values], columns = ['Name', 'Value'])
                result_dataframe = result_dataframe.append(temp_df, ignore_index = True)
...
def convert_back_to_original (result_dataframe, utility_list, storage_list, process_list, bilinear_pieces):
    ...
    for i in range (0, dim_result_dataframe[0]):
        ...
        if (bilin_count == 2 * bilinear_pieces):
            ...
            var1_name, var2_name = find_variable_names(parent_name_actual, utility_list, storage_list, process_list)
...
def find_variable_names (parent_name, utilitylist, storagelist, processlist):
    ##parent_name --- a given name 
    ##utilitylist --- the complete list of utilities
    ##storage_list --- the complete list of storages
    ...
    if check == 0:
        for i in range (0, dim_storagelist[0]):
            if processlist['Name'][i] == parent_name:
                var1_name = storagelist['Variable1'][i]
                var2_name = storagelist['Variable2'][i]
                check = 1
                break          
...
def determine_all_possible_variables(data_required):
    ...
    ##data_required['storagelist_bilinear']
    ##data_required['processlist_bilinear']
    ##data_required['utilitylist_linear']
    ##data_required['storagelist_linear']
    ...
    dim_storagelist_bilinear = data_required['storagelist_bilinear'].shape
    ...
    dim_storagelist_linear = data_required['storagelist_linear'].shape
    ...
    for i in range(0, dim_storagelist_bilinear[0]):
        parent_all_list.append(data_required['storagelist_bilinear']['Name'][i])
        parent_all_list.append(data_required['storagelist_bilinear']['Name'][i] + '_y')   
    ...
    for i in range(0, dim_utilitylist_linear[0]):
        if prev_y_value != data_required['utilitylist_linear']['Parent'][i] + '_y':
            parent_all_list.append(data_required['utilitylist_linear']['Parent'][i] + '_' + data_required['utilitylist_linear']['Name'][i])
            parent_all_list.append(data_required['utilitylist_linear']['Parent'][i] + '_y')
            prev_y_value = data_required['utilitylist_linear']['Parent'][i] + '_y'
        else:
            parent_all_list.append(data_required['utilitylist_linear']['Parent'][i] + '_' + data_required['utilitylist_linear']['Name'][i])            

    for i in range(0, dim_storagelist_linear[0]):
        if prev_y_value != data_required['storagelist_linear']['Parent'][i] + '_y':
            parent_all_list.append(data_required['storagelist_linear']['Parent'][i] + '_' + data_required['storagelist_linear']['Name'][i])
            parent_all_list.append(data_required['storagelist_linear']['Parent'][i] + '_y')
            prev_y_value = data_required['storagelist_linear']['Parent'][i] + '_y'
        else:
            parent_all_list.append(data_required['storagelist_linear']['Parent'][i] + '_' + data_required['storagelist_linear']['Name'][i])            

    for i in range(0, dim_processlist_linear[0]):
        if prev_y_value != data_required['processlist_linear']['Parent'][i] + '_y':
            parent_all_list.append(data_required['processlist_linear']['Parent'][i] + '_' + data_required['processlist_linear']['Name'][i])
            parent_all_list.append(data_required['processlist_linear']['Parent'][i] + '_y')
            prev_y_value = data_required['processlist_linear']['Parent'][i] + '_y'
        else:
            parent_all_list.append(data_required['processlist_linear']['Parent'][i] + '_' + data_required['processlist_linear']['Name'][i])   
