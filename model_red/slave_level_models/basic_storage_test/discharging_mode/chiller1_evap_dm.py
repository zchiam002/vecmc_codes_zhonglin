...
def checktype_chiller1_evap_cm (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller1_evap_cm (ch1_cm_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from chiller1_evap_cm_compute import chiller1_evap_cm_compute
    import pandas as pd
    import numpy as np
    
    ##This is the evaporator side of chiller1_evap_cm
    ...
    ##ch1_cm_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch1_cm_evap_ret_temp = ch1_cm_mdv['Value'][0]
    ch1_cm_ctin = ch1_cm_mdv['Value'][1]
    ch1_cm_tenwkflow = ch1_cm_mdv['Value'][2]                     ##Total flowrate through all evaporators of all chillers  
    ch1_cm_steps = ch1_cm_mdv['Value'][3]                         ##The number of piecewise linear pieces
    
    ##Defined constants 
    ch1_cm_rated_cap = 2000
    ch1_cm_b0 = 0.123020043325872
    ch1_cm_b1 = 1044.79734873891
    ch1_cm_b2 = 0.0204660495029597
    ch1_cm_qc_coeff = 1.09866273284186
    ch1_cm_cp = 4.2
    ch1_cm_min_flow = 0.5 * 218.3903135                            ##This is to ensure that the minimum flowrate through the evaporator is at least half that oc manufacturer's specifications
    
    ##Calling a compute file to process dependent values
        ##Placing the values in a numpy array for easy data handling 
    ch1_cm_dc = np.zeros((9, 1))
    ch1_cm_dc[0,0] = ch1_cm_evap_ret_temp
    ch1_cm_dc[1,0] = ch1_cm_ctin
    ch1_cm_dc[2,0] = ch1_cm_rated_cap
    ch1_cm_dc[3,0] = ch1_cm_b0
    ch1_cm_dc[4,0] = ch1_cm_b1
    ch1_cm_dc[5,0] = ch1_cm_b2
    ch1_cm_dc[6,0] = ch1_cm_qc_coeff
    ch1_cm_dc[7,0] = ch1_cm_tenwkflow
    ch1_cm_dc[8,0] = ch1_cm_steps
    
    ch1_cm_ret_vals = chiller1_evap_cm_compute(ch1_cm_dc)
    
#################################################################################################################################################################################################        
    ##Unit definition 
    
    ##Evaporator stepwise
    for i in range (0, int(ch1_cm_steps)):
        ud = {}
        ud['Name'] = 'ch1_cm_' + str(i + 1) + '_evap'
        ...
        ud['Coeff_v1_1'] = ((ch1_cm_tenwkflow * ch1_cm_cp * 998.2 * (ch1_cm_evap_ret_temp - 273.15 - 1) / 3600))     ##The relationship is Qe = m_evap_perc*m_total*Cp*Tevap_in - m_evap_perc*m_total*Cp*Tevap_out
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ud['Coeff_v1_v2'] = -((ch1_cm_tenwkflow * ch1_cm_cp * 998.2 * (ch1_cm_evap_ret_temp - 273.15 - 1)/ 3600)) 
        ud['Coeff_cst'] = 0
        ud['Fmin'] = ch1_cm_ret_vals['lb'][i] * ch1_cm_rated_cap
        ud['Fmax'] = ch1_cm_ret_vals['ub'][i] * ch1_cm_rated_cap
        ud['Cost_v1_2'] = 0
        ud['Cost_v1_1'] = 0
        ud['Cost_v2_2'] = 0
        ud['Cost_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = (ch1_cm_ret_vals['grad'][i] * ch1_cm_tenwkflow * 4.2 * (ch1_cm_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = 0
        ud['Power_v1_v2'] = -(ch1_cm_ret_vals['grad'][i] * ch1_cm_tenwkflow * 4.2 * (ch1_cm_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        ud['Power_cst'] = ch1_cm_ret_vals['int'][i]
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch1_cm_steps)):
        ...
        ##Stream --- temperature 
        stream = {}                         
        stream['Parent'] = 'ch1_cm_' + str(i + 1) + '_evap'
        stream['Type'] = 'temp_chil'
        stream['Name'] = 'ch1_cm_' + str(i + 1) + '_evap_tout'
        ...
        stream['Stream_coeff_v1_v2'] = (ch1_cm_evap_ret_temp - 273.15 - 1)
        ...
        stream['Parent'] = 'ch1_cm_' + str(i + 1) + '_evap'
        stream['Type'] = 'flow'
        stream['Name'] = 'ch1_cm_' + str(i + 1) + '_evap_mfout'
        stream['Layer'] = 'ch1_2_ch1evapnwk_flow'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ch1_cm_tenwkflow 
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        ...
        stream['Name'] = 'ch1_cm_' + str(i + 1) + '_evap_energyout'
        stream['Layer'] = 'ch1_evap2cond'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ((ch1_cm_tenwkflow * ch1_cm_cp * 998.2 * (ch1_cm_evap_ret_temp - 273.15 - 1) / 3600)) + (ch1_cm_ret_vals['grad'][i] * ch1_cm_tenwkflow * 4.2 * (ch1_cm_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = -((ch1_cm_tenwkflow * ch1_cm_cp * 998.2 * (ch1_cm_evap_ret_temp - 273.15 - 1)/ 3600)) - (ch1_cm_ret_vals['grad'][i] * ch1_cm_tenwkflow * 4.2 * (ch1_cm_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        stream['Stream_coeff_cst'] = ch1_cm_ret_vals['int'][i]
        stream['InOut'] = 'out'
...
############################################################################################################################################################################################################
    ...
    eqn = {}
    eqn['Name'] = 'totaluse_ch1_e_c_cm'                                        ##To make sure that if the evaporator is operated, the condenser is also
    eqn['Type'] = 'unit_binary'
    eqn['Sign'] = 'equal_to'
    eqn['RHS_value'] = 0
    ...
    for i in range (0, int(ch1_cm_steps)):   
        term = {}
        term['Parent_unit'] = 'ch1_cm_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'totaluse_ch1_e_cm'
        ...
        term = {}
        term['Parent_unit'] = 'ch1_cm_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'ch1_e_cm_flow_min'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = ch1_cm_tenwkflow
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 0
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = -ch1_cm_min_flow
        ...
        term = {}
        term['Parent_unit'] = 'ch1_cm_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'totaluse_ch1_e_c_cm'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 1
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = 0
