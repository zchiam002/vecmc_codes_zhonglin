...
def checktype_evap_pump3 (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def evap_pump3 (ep3_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from evap_pump3_compute import evap_pump3_compute
    import pandas as pd
    import numpy as np
    ...
    ##ep3_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ep3_steps = ep3_mdv['Value'][0]
    ep3_time_dependent_elect_tariff = ep3_mdv['Value'][1]   ##Time dependent electricity tariff per kWh used. 

    ##Defined constants 
    ep3_c0 = -0.0000136254       ##These are pump cure coefficients which are needed to formulate constraints                   
    ep3_c1 = 0.0001647403
    ep3_c2 = 21.4327511013
    
    ##These values are pre-calculated and needs to be extracted from a specific location 
     
    ep3_p_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\load_only_storage_milp_master_level_models\\look_up_tables\\'      ##Distribution pump look up table location 
    ep3_p_table_filename = 'evap_cond_pump_lincoeff.csv'
    ep3_p_table = pd.read_csv(ep3_p_look_up_table_loc + ep3_p_table_filename)
    
    ep3_p_coeff = ep3_p_table['p_coeff'][1]
    ep3_m_coeff = ep3_p_table['m_coeff'][1]
    ep3_cst = ep3_p_table['cst'][1]
    ep3_max_m = ep3_p_table['max_flow'][1]
    ep3_maxpressureatmaxrpm = ep3_p_table['max_pressure'][1]
    ...
    ep3_calc = evap_pump3_compute(ep3_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ep3_steps)):
        ud = {}
        ud['Name'] = 'ep3_' + str(i + 1)
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = 'delp'                                                                  
        ud['Fmin_v1'] = ep3_calc['lb'][i] * ep3_max_m
        ud['Fmax_v1'] = ep3_calc['ub'][i] * ep3_max_m                                                                              
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = ep3_maxpressureatmaxrpm                                                                                  
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ...
        ud['Fmax'] = 0
        ud['Cost_v1_2'] = 0
        ud['Cost_v1_1'] = ep3_time_dependent_elect_tariff * ep3_m_coeff
        ud['Cost_v2_2'] = 0
        ud['Cost_v2_1'] = ep3_time_dependent_elect_tariff * ep3_p_coeff
        ud['Cost_v1_v2'] = 0
        ud['Cost_cst'] = ep3_time_dependent_elect_tariff * ep3_cst
        ud['Cinv_v1_2'] = 0
        ud['Cinv_v1_1'] = 0
        ud['Cinv_v2_2'] = 0
        ud['Cinv_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = ep3_m_coeff
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = ep3_p_coeff
        ud['Power_v1_v2'] = 0
        ud['Power_cst'] = ep3_cst
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ep3_steps)):
        ...
        ##Stream --- flowrate into the chiller 3 evaporator pump from chiller 3  
        stream = {}                         
        stream['Parent'] = 'ep3_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ep3_' + str(i + 1) + '_flow_in'
        ...
        ##Stream --- pressure from chiller 3 and evaporator network to chiller 3 evaporator pump 
        stream = {}                                                                
        stream['Parent'] = 'ep3_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'ep3_' + str(i + 1) + '_delp_in'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ep3_steps)):
        eqn = {}
        eqn['Name'] = 'ep3_' + str(i + 1) + '_max_pressure'
        eqn['Type'] = 'stream_limit_modified'                                      
        eqn['Sign'] = 'less_than_equal_to'                                    
        eqn['RHS_value'] = ep3_calc['int'][i]
    ...
    for i in range (0, int(ep3_steps)):   
        term = {}
        term['Parent_unit'] = 'ep3_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ep3'
        ...
        term = {}
        term['Parent_unit'] = 'ep3_' + str(i + 1)
        term['Parent_eqn'] = 'ep3_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = -ep3_calc['grad'][i]
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 1
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = 0
