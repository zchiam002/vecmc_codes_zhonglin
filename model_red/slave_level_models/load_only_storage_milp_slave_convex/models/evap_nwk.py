## This is a evaporator network model, formulated as an input to a quadratic program
...
def checktype_evap_nwk (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def evap_nwk (enwk_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from evap_nwk_compute import evap_nwk_compute
    import pandas as pd
    import numpy as np

    ##This model represents the part of the common pipe in the evaporator network which influences the operating point of all evaporator pumps
    
    ##enwk_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    enwk_steps = enwk_mdv['Value'][0]         
    
    ##Defined constants 
    enwk_com_coeff = 1.66667E-05
    enwk_max_flow = 2300                                                     ##This upper bound is needed for the computation of stepwise pressure
    ...
    enwk_calc = evap_nwk_compute(enwk_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(enwk_steps)):
        ud = {}
        ud['Name'] = 'enwk_' + str(i + 1)
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = '-'                                                                  
        ud['Fmin_v1'] = enwk_calc['lb'][i] * enwk_max_flow
        ud['Fmax_v1'] = enwk_calc['ub'][i] * enwk_max_flow                                                                           
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = 0                                                                                   
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(enwk_steps)):
        ...
        ##Stream --- flowrate into the evaporator network from the chillers
        stream = {}                         
        stream['Parent'] = 'enwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'enwk_' + str(i + 1) + '_flow_in'
        ...
        ##Stream --- pressure stream exiting evaporator network into evaporator pump 1
        stream = {}                                                                
        stream['Parent'] = 'enwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'enwk_' + str(i + 1) + '_pressure_out_ep1'
        stream['Layer'] = 'e_nwk_2_ep1'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = enwk_calc['grad'][i] 
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = 0
        stream['Stream_coeff_cst'] = enwk_calc['int'][i]
        stream['InOut'] = 'out'
        ...
        stream['Parent'] = 'enwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'enwk_' + str(i + 1) + '_pressure_out_ep2'
        ...
        stream['Parent'] = 'enwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'enwk_' + str(i + 1) + '_pressure_out_ep3'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(enwk_steps)):   
        term = {}
        term['Parent_unit'] = 'enwk_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_enwk'
