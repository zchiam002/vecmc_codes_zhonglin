...
def checktype_gv2_nwk (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def gv2_nwk (gv2_nwk_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from gv2_nwk_compute import gv2_nwk_compute
    import pandas as pd
    import numpy as np

    ##This model represents the gv2 network model
    
    ##gv2_nwk_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    gv2_nwk_steps = gv2_nwk_mdv['Value'][0]
    gv2_nwk_tf = gv2_nwk_mdv['Value'][1]
    gv2_nwk_max_flow = gv2_nwk_mdv['Value'][1] * 0.9                     ##This upper bound is needed for the computation of stepwise pressure
    ...
    gv2_nwk_calc = gv2_nwk_compute(gv2_nwk_dc)
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(gv2_nwk_steps)):
        ...
        stream = {}                         
        stream['Parent'] = 'gv2_nwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'gv2_nwk_' + str(i + 1) + '_flow_in'
        ...
        stream['Parent'] = 'gv2_nwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'gv2_nwk_' + str(i + 1) + '_flow_out'
        ...
        stream['Name'] = 'gv2_nwk_' + str(i + 1) + '_delp_out'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(gv2_nwk_steps)):   
        term = {}
        term['Parent_unit'] = 'gv2_nwk_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_gv2_nwk'
