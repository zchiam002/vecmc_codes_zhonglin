...
def checktype_chiller3_evap (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller3_evap (ch3_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from chiller3_evap_compute import chiller3_evap_compute
    import pandas as pd
    import numpy as np
    
    ##This is the evaporator side of chiller3_evap
    ...
    ch3_cp = 4.2
    ch3_min_flow = 0.5 * 820.4531779
    
    ##Calling a compute file to process dependent values
        ##Placing the values in a numpy array for easy data handling 
    ch3_dc = np.zeros((10, 1))
    ch3_dc[0,0] = ch3_evap_ret_temp
    ch3_dc[1,0] = ch3_ctin
    ch3_dc[2,0] = ch3_rated_cap
    ch3_dc[3,0] = ch3_b0
    ch3_dc[4,0] = ch3_b1
    ch3_dc[5,0] = ch3_b2
    ch3_dc[6,0] = ch3_qc_coeff
    ch3_dc[7,0] = ch3_tenwkflow
    ch3_dc[8,0] = ch3_steps
    
    ch3_ret_vals = chiller3_evap_compute(ch3_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch3_steps)):
        ...
        ud['Coeff_v1_1'] = ((ch3_tenwkflow * ch3_cp * 998.2 * (ch3_evap_ret_temp - 273.15 - 1) / 3600))     ##The relationship is Qe = m_evap_perc*m_total*Cp*Tevap_in - m_evap_perc*m_total*Cp*Tevap_out
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ud['Coeff_v1_v2'] = -((ch3_tenwkflow * ch3_cp * 998.2 * (ch3_evap_ret_temp - 273.15 - 1)/ 3600)) 
        ud['Coeff_cst'] = 0
        ud['Fmin'] = ch3_ret_vals['lb'][i] * ch3_rated_cap
        ud['Fmax'] = ch3_ret_vals['ub'][i] * ch3_rated_cap
        ud['Cost_v1_2'] = 0
        ud['Cost_v1_1'] = 0
        ud['Cost_v2_2'] = 0
        ud['Cost_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = (ch3_ret_vals['grad'][i] * ch3_tenwkflow * 4.2 * (ch3_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = 0
        ud['Power_v1_v2'] = -(ch3_ret_vals['grad'][i] * ch3_tenwkflow * 4.2 * (ch3_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch3_steps)):
        ...
        ##Stream --- temperature 
        stream = {}                         
        stream['Parent'] = 'ch3_' + str(i + 1) + '_evap'
        stream['Type'] = 'temp_chil'
        stream['Name'] = 'ch3_' + str(i + 1) + '_evap_tout'
        ...
        stream['Stream_coeff_v1_v2'] = (ch3_evap_ret_temp - 273.15 - 1)
        ...
        stream['Parent'] = 'ch3_' + str(i + 1) + '_evap'
        stream['Type'] = 'flow'
        stream['Name'] = 'ch3_' + str(i + 1) + '_evap_mfout'
        stream['Layer'] = 'ch3_2_ch3evapnwk_flow'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ch3_tenwkflow 
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch3_steps)):   
        term = {}
        term['Parent_unit'] = 'ch3_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'totaluse_ch3_e'
        ...
        term = {}
        term['Parent_unit'] = 'ch3_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'ch3_e_flow_min'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = ch3_tenwkflow
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 0
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = -ch3_min_flow
