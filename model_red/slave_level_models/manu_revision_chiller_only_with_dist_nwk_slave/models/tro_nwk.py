## This is a tro network model, formulated as an input to a quadratic program
...
def checktype_tro_nwk (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def tro_nwk (tro_nwk_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from tro_nwk_compute import tro_nwk_compute
    import pandas as pd
    import numpy as np

    ##This model represents the tro network before the split to pfa and ser
    
    ##tro_nwk_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    tro_nwk_steps = tro_nwk_mdv['Value'][0]
    tro_nwk_tf = tro_nwk_mdv['Value'][1]
    tro_nwk_max_flow = tro_nwk_mdv['Value'][1]  * 0.5                 ##This upper bound is needed for the computation of stepwise pressure
    
    ##Defined constants 
    tro_nwk_coeff = 0.00043901081005616
    ...
    tro_nwk_calc = tro_nwk_compute(tro_nwk_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(tro_nwk_steps)):
        ud = {}
        ud['Name'] = 'tro_nwk_' + str(i + 1)
        ud['Variable1'] = 'm_perc'                                                                        
        ud['Variable2'] = '-'                                                                  
        ud['Fmin_v1'] = tro_nwk_calc['lb'][i]
        ud['Fmax_v1'] = tro_nwk_calc['ub'][i]                                                                                
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = 0                                                                                   
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(tro_nwk_steps)):
        ...
        stream = {}                         
        stream['Parent'] = 'tro_nwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'tro_nwk_' + str(i + 1) + '_flow_in'
        stream['Layer'] = 'evap_nwk_flow'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = tro_nwk_tf
        ...
        ##Stream --- flowrate out of the ice network into pfa and ser networks
        stream = {}                                                                
        stream['Parent'] = 'tro_nwk_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'tro_nwk_' + str(i + 1) + '_flow_out'
        ...
        ##Stream --- pressure flow from tro network to pfa pressure layer
        stream = {}                                                                
        stream['Parent'] = 'tro_nwk_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'tro_nwk_' + str(i + 1) + '_2_pfa_nwk_delp_out'
        stream['Layer'] = 'pfa_consol_delp'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = tro_nwk_tf * tro_nwk_calc['grad'][i]
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = 0
        stream['Stream_coeff_cst'] = tro_nwk_calc['int'][i]
        stream['InOut'] = 'out'
        ...
        stream['Parent'] = 'tro_nwk_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'tro_nwk_' + str(i + 1) + '_2_ser_nwk_delp_out'
        stream['Layer'] = 'ser_consol_delp'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = tro_nwk_tf * tro_nwk_calc['grad'][i]
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(tro_nwk_steps)):   
        term = {}
        term['Parent_unit'] = 'tro_nwk_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_tro_nwk'
