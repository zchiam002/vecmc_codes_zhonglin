...
def checktype_chiller2_cond_pump_4nc (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller2_cond_pump_4nc (ch2_cp_4nc_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from chiller2_cond_pump_4nc_compute import chiller2_cond_pump_4nc_compute
    import pandas as pd
    import numpy as np
    ...
    ##ch2_cp_4nc_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch2_cp_steps = ch2_cp_4nc_mdv['Value'][0]

    ##Defined constants 
    ch2_cp_c0 = -0.0000090818       ##These are pump cure coefficients which are needed to formulate constraints                   
    ch2_cp_c1 = 0.0029568794
    ch2_cp_c2 = 45.1880038403
    
    ##These values are pre-calculated and needs to be extracted from a specific location 
     
    ch2_cp_p_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\paper_qe_testcase_master_level_models\\look_up_tables\\'      ##Distribution pump look up table location 
    ch2_cp_p_table_filename = 'evap_cond_pump_lincoeff.csv'
    ch2_cp_p_table = pd.read_csv(ch2_cp_p_look_up_table_loc + ch2_cp_p_table_filename)
    
    ch2_cp_p_coeff = ch2_cp_p_table['p_coeff'][4]
    ch2_cp_m_coeff = ch2_cp_p_table['m_coeff'][4]
    ch2_cp_cst = ch2_cp_p_table['cst'][4]
    ch2_cp_max_m = ch2_cp_p_table['max_flow'][4]
    ch2_cp_maxpressureatmaxrpm = ch2_cp_p_table['max_pressure'][4]
    ...
    ch2_cp_calc = chiller2_cond_pump_4nc_compute(ch2_cp_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch2_cp_steps)):
        ud = {}
        ud['Name'] = 'ch2_cp_4nc_' + str(i + 1)
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = 'delp'                                                                  
        ud['Fmin_v1'] = ch2_cp_calc['lb'][i] * ch2_cp_max_m
        ud['Fmax_v1'] = ch2_cp_calc['ub'][i] * ch2_cp_max_m                                                                              
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = ch2_cp_maxpressureatmaxrpm                                                                                  
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = ch2_cp_m_coeff
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = ch2_cp_p_coeff
        ud['Power_v1_v2'] = 0
        ud['Power_cst'] = ch2_cp_cst
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch2_cp_steps)):
        ...
        ##Stream --- flowrate into the chiller 2 condenser pump from chiller 2 condenser network 
        stream = {}                         
        stream['Parent'] = 'ch2_cp_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch2_cp_4nc_' + str(i + 1) + '_flow_in'
        stream['Layer'] = 'ch2condnwk_2_ch2condpump_flow' 
        ...
        ##Stream --- pressure from chiller 2 condenser network to chiller 2 condenser pump
        stream = {}                                                                
        stream['Parent'] = 'ch2_cp_4nc_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'ch2_cp_4nc_' + str(i + 1) + '_delp_in'
        stream['Layer'] = 'ch2condnwk_2_ch2condpump_delp'
        stream['Stream_coeff_v1_2'] = 0
        ...
        ##Stream --- flowrate from chiller 2 condenser pump to distribution network
        stream = {}                                                                
        stream['Parent'] = 'ch2_cp_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch2_cp_4nc_' + str(i + 1) + '_flow_out'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch2_cp_steps)):
        eqn = {}
        eqn['Name'] = 'ch2_cp_4nc_' + str(i + 1) + '_max_pressure'
        eqn['Type'] = 'stream_limit_modified'                                      
        eqn['Sign'] = 'less_than_equal_to'                                    
        eqn['RHS_value'] = ch2_cp_calc['int'][i]
    ...
    for i in range (0, int(ch2_cp_steps)):   
        term = {}
        term['Parent_unit'] = 'ch2_cp_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ch2_cp_4nc'
        ...
        term = {}
        term['Parent_unit'] = 'ch2_cp_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'ch2_cp_4nc_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = -ch2_cp_calc['grad'][i]
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 1
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = 0
