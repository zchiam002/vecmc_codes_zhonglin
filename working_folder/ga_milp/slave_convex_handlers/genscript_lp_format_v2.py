##This is version 2, added storage capabilities

def genscript_lp_format_v2 (input_dataframes, utilitylist, storagelist, processlist, layerslist, parallel_thread_num, obj_func, bilinear_pieces):
...
###############################################################################################################################################################################################
    ...
    ##input_dataframes['storagelist_bilinear']                  --- list of bilinear storages
    ...
    ##input_dataframes['storagelist_linear']                    --- linear storage list
...
###############################################################################################################################################################################################    
    ...
    f_data_set.write('\\\\ Objective function for all utilities, storages and processes \n \n')
    ...
    ##Writing the storage terms in the objective function for strictly linear terms
    dim_storagelist_linear = input_dataframes['storagelist_linear'].shape
   
    prev_y_name = ''                                                            ##For the sake of GLPK                                     
    
    for i in range (0, dim_storagelist_linear[0]):
        var_name_temp = input_dataframes['storagelist_linear']['Parent'][i] + '_' + input_dataframes['storagelist_linear']['Name'][i]
        
        if prev_y_name != input_dataframes['storagelist_linear']['Parent'][i] + '_y':
            var_name_temp_y = input_dataframes['storagelist_linear']['Parent'][i] + '_y'
            prev_y_name = input_dataframes['storagelist_linear']['Parent'][i] + '_y'
            
            grad_temp = input_dataframes['storagelist_linear'][key + '1'][i]
            int_temp = input_dataframes['storagelist_linear'][key + '2'][i]
    ...
    ##Writing the storage terms in the objective function for relaxed bilinear terms 
    dim_storagelist_bilinear = input_dataframes['storagelist_bilinear'].shape

    for i in range (0, dim_storagelist_bilinear[0]):           
        var_name_temp = input_dataframes['storagelist_bilinear']['Name'][i]
        var_name_temp_y = var_name_temp + '_y'
        grad_temp = input_dataframes['storagelist_bilinear'][key + '1'][i]
        int_temp = input_dataframes['storagelist_bilinear'][key + '2'][i]       
...
###############################################################################################################################################################################################    
    ...
    ##Writing Fmin, Fmax constraints for storages linear 
    f_data_set.write('\\\\Writing Fmin, Fmax constraints for storages linear \n \n')
    
    for i in range (0, dim_storagelist_linear[0]):
        var_name_temp = input_dataframes['storagelist_linear']['Parent'][i] + '_' + input_dataframes['storagelist_linear']['Name'][i]
        var_name_temp_y = input_dataframes['storagelist_linear']['Parent'][i] + '_y'
        fmin_temp = input_dataframes['storagelist_linear']['Fmin'][i]
        fmax_temp = input_dataframes['storagelist_linear']['Fmax'][i]
    ...
    ##Writing Fmin, Fmax constraints for storages bilinear 
    f_data_set.write('\\\\Writing Fmin, Fmax constraints for storages bilinear \n \n')
    
    for i in range (0, dim_storagelist_bilinear[0]):
        var_name_temp = input_dataframes['storagelist_bilinear']['Name'][i]
        var_name_temp_y = var_name_temp + '_y'
        fmin_temp = input_dataframes['storagelist_bilinear']['Fmin'][i]
        fmax_temp = input_dataframes['storagelist_bilinear']['Fmax'][i]
    ...
    ##Writing the fmin and fmax constraint in relation to x and y for storages 
    f_data_set.write('\\\\Writing the fmin and fmax constraint in relation to x and y for storages \n \n')
    dim_storagelist = storagelist.shape
    
    check_list = input_dataframes['storagelist_bilinear']['Parent'][:]
    
    for i in range (0, dim_storagelist[0]):
        check_value = check_if_item_in_list (check_list, storagelist['Name'][i])
        if  check_value == 1:            
            base_parent_name_onoff = storagelist['Name'][i] + '_y'
            
            x_fmin_value = storagelist['Fmin_v1'][i] * 2
            x_fmax_value = storagelist['Fmax_v1'][i] * 2
            y_fmin_value = storagelist['Fmin_v2'][i] * 2
            y_fmax_value = storagelist['Fmax_v2'][i] * 2
            ...
            for j in range (0, bilinear_pieces):
                base_name_u = storagelist['Name'][i] + '_u' + str(j)
                base_name_v = storagelist['Name'][i] + '_v' + str(j)  
    ...
    for i in range (0, dim_processlist[0]):
        ...
        if  check_value == 1:
            ...
            temp_term = ''
            temp_term = temp_term + 'c' + str(current_index) + ': '
            temp_term = temp_term + u_term_temp_pos + ' - ' + u_term_temp_neg + ' <= ' + str(y_fmax_value)
            f_data_set.write(temp_term + '\n')  
...
###############################################################################################################################################################################################
    ...
    ##Writing the binary variables for all the parent utilities and storages
    ...
    for i in range (0, dim_storagelist[0]):
        temp_term = ''
        temp_term = storagelist['Name'][i] + '_y'
        f_data_set.write(temp_term + '\n')          

    for i in range (0, dim_storagelist_bilinear[0]):
        temp_term = ''
        temp_term = input_dataframes['storagelist_bilinear']['Name'][i] + '_y'
        f_data_set.write(temp_term + '\n')   
