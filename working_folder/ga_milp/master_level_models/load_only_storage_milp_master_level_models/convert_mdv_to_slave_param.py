...
def convert_mdv_to_slave_param_v2 (thread_number, var, demand, weather_and_ct_coeff, piecewise_steps):
    ...
    import sys 
    sys.path.append('C:\\Optimization_zlc\\master_level_models\\load_only_storage_milp_master_level_models\\prep_functions\\')
    import os.path
...
##Version 3 is created to handle multi-time problems 
def convert_mdv_to_slave_param_v3 (thread_number, var, demand, weather, elect_tariff, piecewise_steps, time_steps):
    
    import sys 
    sys.path.append('C:\\Optimization_zlc\\master_level_models\\load_only_storage_milp_master_level_models\\prep_functions\\')
    import os.path
    from prepare_slave_param_csv_vtest3 import prepare_slave_param_csv_vtest3    #Needs modification
    import pandas as pd    
    
    ##thread_number --- the thread number for coding output files
    ##var --- variable values from the master decision 
    ##demand --- the demand of the substations
    ##weather --- the ambient temperature information
    ##elect_tariff --- the pricing of electricity variation per hour
    ##piecewise_steps --- the number of piecewise_steps 
    ##time_steps --- the number of time-steps involved in the multi-time problem 
    ...
    ##Putting the values into a dataframe
    mdv_slave_param = prepare_slave_param_csv_vtest3(var, demand, weather, elect_tariff, piecewise_steps, time_steps)
