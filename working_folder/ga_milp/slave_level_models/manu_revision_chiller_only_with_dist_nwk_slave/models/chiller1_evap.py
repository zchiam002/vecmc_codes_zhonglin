...
def checktype_chiller1_evap (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller1_evap (ch1_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from chiller1_evap_compute import chiller1_evap_compute
    import pandas as pd
    import numpy as np
    
    ##This is the evaporator side of chiller1_evap
    ...
    ch1_steps = ch1_mdv['Value'][3]                         ##The number of piecewise linear pieces
    ...
    ch1_cp = 4.2
    ch1_min_flow = 0.5 * 218.3903135                            ##This is to ensure that the minimum flowrate through the evaporator is at least half that oc manufacturer's specifications
    ...
    ch1_ret_vals = chiller1_evap_compute(ch1_dc)
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch1_steps)):   
        ...
        term['Parent_eqn'] = 'totaluse_ch1_e'
        ...
        term = {}
        term['Parent_unit'] = 'ch1_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'ch1_e_flow_min'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
