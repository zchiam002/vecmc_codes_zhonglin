...
def checktype_chiller1_nwk_choice_4 (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller1_nwk_choice_4 (ch1_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from chiller1_compute_nwk_choice_4 import chiller1_compute_nwk_choice_4
    import pandas as pd
    import numpy as np
    ...
    ch1_evap_ret_temp = ch1_mdv['Value'][0]
    ...
    ch1_steps = ch1_mdv['Value'][4]                        ##The number of piecewise linear pieces
    ...
    ch1_dc[8,0] = ch1_tcnwkflow
    ch1_dc[9,0] = ch1_steps
    
    ch1_ret_vals = chiller1_compute_nwk_choice_4(ch1_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch1_steps)):
        ...
        ud['Fmin_v2'] = 273.15 + 1                                                      ##The minimum supply temperature of the chiller is 1 deg 
        ud['Fmax_v2'] = ch1_evap_ret_temp                                                    ##The maximum supply temperature of the chiller is that of the return temperature
        ud['Coeff_v1_2'] = 0                                                            ##This illustrates the relationship between the variables  
        ud['Coeff_v1_1'] = ch1_tenwkflow * 998.2 * ch1_cp * ch1_evap_ret_temp / 3600         ##The relationship is Qe = m_evap_perc*m_total*Cp*Tevap_in - m_evap_perc*m_total*Cp*Tevap_out
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ud['Coeff_v1_v2'] = -ch1_tenwkflow * ch1_cp * 998.2 / 3600
        ud['Coeff_cst'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = ch1_ret_vals['grad'][i] * ch1_tenwkflow * 4.2 * ch1_evap_ret_temp * 998.2 / 3600
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = 0
        ud['Power_v1_v2'] = ch1_ret_vals['grad'][i] * ch1_tenwkflow * 4.2 * 998.2 / 3600
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch1_steps)):
        ...
        stream['Parent'] = 'ch1_' + str(i + 1) + '_evap'
        stream['Type'] = 'balancing_only'
        stream['Name'] = 'ch1_' + str(i + 1) + '_evap_mfout'
        stream['Layer'] = 'chil2gv2_flow'
        stream['Stream_coeff_v1_2'] = 0
        ...
        ##Stream --- flowrate_check 
        stream = {}                                                                ##To make sure that totalflowrate tallies 
        stream['Parent'] = 'ch1_' + str(i + 1) + '_evap'
        stream['Type'] = 'balancing_only'
        stream['Name'] = 'ch1_' + str(i + 1) + '_evap_mfout_chk'
        stream['Layer'] = 'chil_flow_check'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ch1_tenwkflow
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch1_steps)):   
        term = {}
        term['Parent_unit'] = 'ch1_' + str(i + 1) + '_evap'
