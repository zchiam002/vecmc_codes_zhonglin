...
def checktype_evap_pump1 (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def evap_pump1 (ep1_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from evap_pump1_compute import evap_pump1_compute
    import pandas as pd
    import numpy as np
    ...
    ##ep1_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ep1_steps = ep1_mdv['Value'][0]
    ep1_time_dependent_elect_tariff = ep1_mdv['Value'][1]   ##Time dependent electricity tariff per kWh used. 

    ##Defined constants 
    ep1_c0 = -0.0001266405       ##These are pump cure coefficients which are needed to formulate constraints                   
    ep1_c1 = 0.0112272822
    ep1_c2 = 12.3463827922
    
    ##These values are pre-calculated and needs to be extracted from a specific location 
     
    ep1_p_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\load_only_storage_milp_master_level_models\\look_up_tables\\'      ##Distribution pump look up table location 
    ep1_p_table_filename = 'evap_cond_pump_lincoeff.csv'
    ep1_p_table = pd.read_csv(ep1_p_look_up_table_loc + ep1_p_table_filename)
    
    ep1_p_coeff = ep1_p_table['p_coeff'][0]
    ep1_m_coeff = ep1_p_table['m_coeff'][0]
    ep1_cst = ep1_p_table['cst'][0]
    ep1_max_m = ep1_p_table['max_flow'][0]
    ep1_maxpressureatmaxrpm = ep1_p_table['max_pressure'][0]
    ...
    ep1_calc = evap_pump1_compute(ep1_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ep1_steps)):
        ud = {}
        ud['Name'] = 'ep1_' + str(i + 1)
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = 'delp'                                                                  
        ud['Fmin_v1'] = ep1_calc['lb'][i] * ep1_max_m
        ud['Fmax_v1'] = ep1_calc['ub'][i] * ep1_max_m                                                                              
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = ep1_maxpressureatmaxrpm                                                                                  
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ...
        ud['Fmax'] = 0
        ud['Cost_v1_2'] = 0
        ud['Cost_v1_1'] = ep1_time_dependent_elect_tariff * ep1_m_coeff
        ud['Cost_v2_2'] = 0
        ud['Cost_v2_1'] = ep1_time_dependent_elect_tariff * ep1_p_coeff
        ud['Cost_v1_v2'] = 0
        ud['Cost_cst'] = ep1_time_dependent_elect_tariff * ep1_cst
        ud['Cinv_v1_2'] = 0
        ud['Cinv_v1_1'] = 0
        ud['Cinv_v2_2'] = 0
        ud['Cinv_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = ep1_m_coeff
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = ep1_p_coeff
        ud['Power_v1_v2'] = 0
        ud['Power_cst'] = ep1_cst
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ep1_steps)):
        ...
        ##Stream --- flowrate into the chiller 1 evaporator pump from chiller 1  
        stream = {}                         
        stream['Parent'] = 'ep1_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ep1_' + str(i + 1) + '_flow_in'
        ...
        ##Stream --- pressure from chiller 1 and evaporator network to chiller 1 evaporator pump 
        stream = {}                                                                
        stream['Parent'] = 'ep1_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'ep1_' + str(i + 1) + '_delp_in'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ep1_steps)):
        eqn = {}
        eqn['Name'] = 'ep1_' + str(i + 1) + '_max_pressure'
        eqn['Type'] = 'stream_limit_modified'                                      
        eqn['Sign'] = 'less_than_equal_to'                                    
        eqn['RHS_value'] = ep1_calc['int'][i]
    ...
    for i in range (0, int(ep1_steps)):   
        term = {}
        term['Parent_unit'] = 'ep1_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ep1'
        ...
        term = {}
        term['Parent_unit'] = 'ep1_' + str(i + 1)
        term['Parent_eqn'] = 'ep1_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = -ep1_calc['grad'][i]
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 1
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = 0
