...
def checktype_dist_nwk_pump (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def dist_nwk_pump (dist_np_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from dist_nwk_pump_compute import dist_nwk_pump_compute
    import pandas as pd
    import numpy as np
    ...
    ##dist_np_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    dist_np_steps = dist_np_mdv['Value'][0]
    dist_np_time_dependent_elect_tariff = dist_np_mdv['Value'][1]   ##Time dependent electricity tariff per kWh used. 

    ##Defined constants
    dist_np_nwk_choice = 3                                                      ##For the sake of pointing to the right pump-network combination, temporary unused
    
    ##The constants are chosen based on predefined pumps combinations
     
    dist_np_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\load_only_storage_milp_master_level_models\\look_up_tables\\'      ##Distribution pump look up table location 
    ...
    ret_values_pump = dist_nwk_pump_compute(dist_np_dc, dist_np_table, dist_np_pres_table)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(dim_ret_values_pump[0])):
        ud = {}
        ud['Name'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i])
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = 'delp'                                                                  
        ...
        ud['Fmax'] = 0
        ud['Cost_v1_2'] = 0
        ud['Cost_v1_1'] = dist_np_time_dependent_elect_tariff * ret_values_pump['m_coeff'][i]
        ud['Cost_v2_2'] = 0
        ud['Cost_v2_1'] = dist_np_time_dependent_elect_tariff * ret_values_pump['p_coeff'][i]
        ud['Cost_v1_v2'] = 0
        ud['Cost_cst'] = dist_np_time_dependent_elect_tariff * ret_values_pump['cst'][i]
        ud['Cinv_v1_2'] = 0
        ud['Cinv_v1_1'] = 0
        ud['Cinv_v2_2'] = 0
        ud['Cinv_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(dim_ret_values_pump[0])):
        ...
        stream = {}                         
        stream['Parent'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i])
        stream['Type'] = 'flow'
        stream['Name'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i]) + '_flow_in'
        ...
        stream['Name'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i]) + '_delp_in'
...
############################################################################################################################################################################################################
    ...
    ##So that it does not exceed the upper-limit imposed by piecewise pressure cuts 
    for i in range (0, int(dim_ret_values_pump[0])):
        eqn = {}
        eqn['Name'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i]) + '_max_pressure'
    ...
    for i in range (0, int(dim_ret_values_pump[0])):   
        term = {}
        term['Parent_unit'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i])
        term['Parent_eqn'] = 'totaluse_dist_np'
        ...
        term = {}
        term['Parent_unit'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i])
        term['Parent_eqn'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i]) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
