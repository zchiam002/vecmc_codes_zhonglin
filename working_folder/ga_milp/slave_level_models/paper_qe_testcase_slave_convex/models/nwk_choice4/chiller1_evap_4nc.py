...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch1_4nc_steps)):
        ...
        stream['Name'] = 'ch1_4nc_' + str(i + 1) + '_evap_energyout'
        stream['Layer'] = 'ch1_evap2cond'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ((ch1_4nc_tenwkflow * ch1_4nc_cp * 998.2 * (ch1_4nc_evap_ret_temp - 273.15 - 1) / 3600)) + (ch1_4nc_ret_vals['grad'][i] * ch1_4nc_tenwkflow * 4.2 * (ch1_4nc_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = -((ch1_4nc_tenwkflow * ch1_4nc_cp * 998.2 * (ch1_4nc_evap_ret_temp - 273.15 - 1)/ 3600)) - (ch1_4nc_ret_vals['grad'][i] * ch1_4nc_tenwkflow * 4.2 * (ch1_4nc_evap_ret_temp - 273.15 - 1) * 998.2 / 3600)
        stream['Stream_coeff_cst'] = ch1_4nc_ret_vals['int'][i]
        stream['InOut'] = 'out'
...
############################################################################################################################################################################################################
    ...
    eqn = {}
    eqn['Name'] = 'totaluse_ch1_e_c_4nc'                                        ##To make sure that if the evaporator is operated, the condenser is also
    eqn['Type'] = 'unit_binary'
    eqn['Sign'] = 'equal_to'
    eqn['RHS_value'] = 0
    ...
    for i in range (0, int(ch1_4nc_steps)):   
        ...
        term = {}
        term['Parent_unit'] = 'ch1_4nc_' + str(i + 1) + '_evap'
        term['Parent_eqn'] = 'totaluse_ch1_e_c_4nc'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 1
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = 0
