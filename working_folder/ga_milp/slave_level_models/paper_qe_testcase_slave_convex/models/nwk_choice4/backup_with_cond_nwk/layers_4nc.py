...
def layers_4nc (layerslist):
    ...
    layer = ['energy', 'ch1_evap2cond']                                         ##The energy flow from chiller 1 evaporator to its condenser
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)     

    layer = ['energy', 'ch2_evap2cond']                                         ##The energy flow from chiller 2 evaporator to its condenser
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True) 
    
    layer = ['energy', 'ch3_evap2cond']                                         ##The energy flow from chiller 3 evaporator to its condenser
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)  
    
    ##Chiller condenser side layers 
    layer = ['flow', 'ch1_2_ch1condnwk_flow']                                   ##The flowrate from chiller 1 condenser to the condensation network
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)      
    
    layer = ['flow', 'ch2_2_ch2condnwk_flow']                                   ##The flowrate from chiller 2 condenser to the condensation network
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)  

    layer = ['flow', 'ch3_2_ch3condnwk_flow']                                   ##The flowrate from chiller 3 condenser to the condensation network
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)  


    ##Condenser network to condenser pumps 
    layer = ['flow', 'ch1condnwk_2_ch1condpump_flow']                           ##The flowrate from chiller 1 condenser network to chiller 1 condenser pump
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)  
    ...
    layer = ['flow', 'ch2condnwk_2_ch2condpump_flow']                           ##The flowrate from chiller 2 condenser network to chiller 2 condenser pump
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)  
    ...
    layer = ['flow', 'ch3condnwk_2_ch3condpump_flow']                           ##The flowrate from chiller 3 condenser network to chiller 3 condenser pump
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)  
    ...
    layer = ['balancing_only', 'cond_nwk_flow']                             ##The flowrate consolidation unit for chiller 1 condenser network
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)    
