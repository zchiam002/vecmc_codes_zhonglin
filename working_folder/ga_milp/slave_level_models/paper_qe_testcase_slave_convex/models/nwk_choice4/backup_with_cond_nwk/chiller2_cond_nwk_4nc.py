## This is a chiller 2 condenser network model, formulated as an input to a quadratic program
...
def checktype_chiller2_cond_nwk_4nc (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller2_cond_nwk_4nc (ch2_cnwk_4nc_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    from chiller2_cond_nwk_4nc_compute import chiller2_cond_nwk_4nc_compute
    import pandas as pd
    import numpy as np

    ##This model represents the part of the condenser network responsible for computing the pressure drop faced by chiller 2 condenser pump
    
    ##ch2_cnwk_4nc_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch2_cnwk_steps = ch2_cnwk_4nc_mdv['Value'][0]
    ch2_cnwk_tf = ch2_cnwk_4nc_mdv['Value'][1]                  
    
    ##Defined constants 
    ch2_cnwk_coeff = 0.000017825112553599
    ch2_cnwk_com_coeff = 6.62983425414365E-07
    ch2_cnwk_max_flow = 1600                                                    ##This upper bound is needed for the computation of stepwise pressure
                                                   
    ##Calling a compute file to process dependent values
        ##Placing the values in a numpy array for easy data handling 
    ch2_cnwk_dc = np.zeros((5,1))
    
    ch2_cnwk_dc[0,0] = ch2_cnwk_tf
    ch2_cnwk_dc[1,0] = ch2_cnwk_coeff
    ch2_cnwk_dc[2,0] = ch2_cnwk_com_coeff
    ch2_cnwk_dc[3,0] = ch2_cnwk_max_flow
    ch2_cnwk_dc[4,0] = ch2_cnwk_steps

    ch2_cnwk_calc = chiller2_cond_nwk_4nc_compute(ch2_cnwk_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch2_cnwk_steps)):
        ud = {}
        ud['Name'] = 'ch2_cnwk_4nc_' + str(i + 1)
        ud['Variable1'] = 'm_perc'                                                                        
        ud['Variable2'] = '-'                                                                  
        ud['Fmin_v1'] = ch2_cnwk_calc['lb'][i]
        ud['Fmax_v1'] = ch2_cnwk_calc['ub'][i]                                                                                
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = 0                                                                                   
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch2_cnwk_steps)):
        ...
        ##Stream --- flowrate into the chiller 2 condenser network from chiller 2 
        stream = {}                         
        stream['Parent'] = 'ch2_cnwk_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch2_cnwk_4nc_' + str(i + 1) + '_flow_in'
        stream['Layer'] = 'ch2_2_ch2condnwk_flow' 
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ch2_cnwk_tf
        ...
        ##Stream --- flowrate from chiller 2 condenser network to chiller 2 condenser pump
        stream = {}                                                                
        stream['Parent'] = 'ch2_cnwk_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch2_cnwk_4nc_' + str(i + 1) + '_flow_out'
        ...
        ##Stream --- pressure flow from chiller 2 condenser network to chiller 2 condenser pump
        stream = {}                                                                
        stream['Parent'] = 'ch2_cnwk_4nc_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'ch2_cnwk_4nc_' + str(i + 1) + '_delp_out'
        stream['Layer'] = 'ch2condnwk_2_ch2condpump_delp' 
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ch2_cnwk_tf  * ch2_cnwk_calc['grad'][i]
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = 0
        stream['Stream_coeff_cst'] = ch2_cnwk_calc['int'][i]
        stream['InOut'] = 'out'
...
############################################################################################################################################################################################################
    ...
    eqn = {}
    eqn['Name'] = 'totaluse_ch2_cnwk_4nc'
    ...
    for i in range (0, int(ch2_cnwk_steps)):   
        term = {}
        term['Parent_unit'] = 'ch2_cnwk_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ch2_cnwk_4nc'
