...
def chiller3_evap_pump_4nc_compute (ch3_ep_dc):
    ...
    ch3_ep_calc = pd.DataFrame(columns = ['grad', 'int' ,'lb' ,'ub'])
    
    ##Determining the step size 
    step_size = 1 / ch3_ep_dc[4,0]
    
    for i in range (0, int(ch3_ep_dc[4,0])):
        lb = i * step_size
        ub = (i + 1) * step_size 
        
        flow_lb = lb * ch3_ep_dc[3,0]
        flow_ub = ub * ch3_ep_dc[3,0]
        
        delp_lb = (ch3_ep_dc[0,0] * pow(flow_lb, 2)) + (ch3_ep_dc[1,0] * flow_lb) + (ch3_ep_dc[2,0])    ##This is a downward sloping graph
        delp_ub = (ch3_ep_dc[0,0] * pow(flow_ub, 2)) + (ch3_ep_dc[1,0] * flow_ub) + (ch3_ep_dc[2,0])        
        ...
        ch3_ep_calc = ch3_ep_calc.append(temp_df, ignore_index = True)
