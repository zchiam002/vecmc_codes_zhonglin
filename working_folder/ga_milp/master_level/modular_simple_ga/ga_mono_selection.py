...
def ga_mono_selection (agents_obj_val_fitness, total_binary_len, selection_choice, selection_choice_data, crossover_perc):
    
    ##agents_obj_val_fitness --- values of x, obj_values and fitness values
    ##total_binary_len --- the total length of the binary string involved 
    ...
    if selection_choice == 'roulette_wheel':
        parent_pool, crossover_duty = ga_mono_roulette_wheel (agents_obj_val_fitness, total_binary_len, crossover_perc)
    elif selection_choice == 'tournament_selection':
        parent_pool, crossover_duty = ga_mono_tournament_selection (agents_obj_val_fitness, total_binary_len, crossover_perc, selection_choice_data)
...
def ga_mono_roulette_wheel (agents_obj_val_fitness, total_binary_len, crossover_perc):
    ...
    ##This process will be very slow if the total_binary_len is too long 
    ...
    for i in range (0, selected_parent_size):
        ...
        probability_reference = copied_agents_obj_val_fitness[:, total_binary_len + 1]
...
def ga_mono_tournament_selection (agents_obj_val_fitness, total_binary_len, crossover_perc, selection_choice_data):
