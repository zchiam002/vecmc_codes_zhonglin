...
def cooling_tower3_compute (ct3_dc):
    ...
    ct3_ind_flow = ct3_dc[2,0] / ct3_dc[0,0]                                    ##To calculate the flowrate to each individual cooling tower (m3/h)
    ...
    T_wi = ct3_dc[1,0]
    T_wb = ct3_dc[3,0]
    m_w = ct3_ind_flow * 998.2
    lin_fan_coeff = ct3_dc[10,0]
    delta_t = 0
    ...
    for i in range (0,iterations):
        T_wo = T_wi - delta_t
        X2 = ct3_dc[7,0]
        X = ct3_dc[5,0] + (ct3_dc[9,0]*(T_wi - T_wb))
        C = ct3_dc[4,0] + (ct3_dc[6,0]*(T_wi - T_wb)) + (ct3_dc[8,0]*(pow((T_wi - T_wb),2))) - ((T_wi - T_wo)/(T_wi - T_wb))
    ...
    max_delta_t = ct3_dc[11,0]*lin_coeff + intercept                                                    ##This is the maximum delta t based on max fan power
    ...
    if max_power_at_max_delta_t_based_on_twb < ct3_dc[11,0]:
        fmax = max_power_at_max_delta_t_based_on_twb / ct3_dc[11,0]
    else:
        fmax = 1
        
    ##Dealing with water related issues 
    min_water_cons = ct3_dc[12,0] * ct3_ind_flow * ct3_dc[14,0]                                                     ##This should function as the interceptat 0 utilization level 
    water_cons_grad_coeff = ct3_dc[13,0] * ct3_ind_flow * ct3_dc[14,0] * min(max_delta_t, max_delta_t_based_on_twb) ##This is the multiplicative factor to the delta t

    ##Initialize a new matrix to hold the return values 

    ct3_dc_calc =np.zeros((6,1))
    
    ct3_dc_calc[0,0] = no_flow_delta_t 
    ct3_dc_calc[1,0] = min(max_delta_t, max_delta_t_based_on_twb)
    ct3_dc_calc[2,0] = min(ct3_dc[11,0], max_power_at_max_delta_t_based_on_twb)
    ct3_dc_calc[3,0] = fmax
    ct3_dc_calc[4,0] = min_water_cons
    ct3_dc_calc[5,0] = water_cons_grad_coeff    
