...
def cooling_tower2_4nc_compute (ct2_4nc_dc):
    ...
    twi_min = ct2_4nc_dc[0,0] + ct2_4nc_dc[2,0]         ##The units are in K
    twi_max = twi_min + ct2_4nc_dc[1,0]
    
    ##Determining the delta t coefficients in terms of %ma and %twi
    ma_perc_coeff = (ct2_4nc_dc[7,0] * ct2_4nc_dc[6,0]) + (ct2_4nc_dc[9,0] * ct2_4nc_dc[6,0] * twi_min)
    twi_perc_coeff = ct2_4nc_dc[8,0] * (twi_max - twi_min) 
    ma_twi_perc_coeff = ct2_4nc_dc[9,0] * ct2_4nc_dc[6,0] * (twi_max - twi_min)
    cst_term = ct2_4nc_dc[10,0] + (ct2_4nc_dc[8,0] * twi_min)
    
    ##Determining the minimum outlet temperature with respect to the flowratio
    tout_min_frac = twi_min / ct2_4nc_dc[5,0]
    
    ##Determining the make up water coefficients 
    flow_to_tower = ct2_4nc_dc[4,0] / ct2_4nc_dc[5,0]
    
    water_coeff1 = 0.0027 * flow_to_tower * ct2_4nc_dc[3,0]
    water_coeff2 = (0.01 / 5.6) * flow_to_tower * ct2_4nc_dc[3,0]
