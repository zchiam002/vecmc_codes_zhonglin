...
def checktype_cond_pump3 (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def cond_pump3 (cp3_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from cond_pump3_compute import cond_pump3_compute
    import pandas as pd
    import numpy as np
    ...
    ##cp3_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    cp3_steps = cp3_mdv['Value'][0]
    cp3_time_dependent_elect_tariff = cp3_mdv['Value'][1]   ##Time dependent electricity tariff per kWh used. 

    ##Defined constants 
    cp3_c0 = -0.0000090818       ##These are pump cure coefficients which are needed to formulate constraints                   
    cp3_c1 = 0.0029568794
    cp3_c2 = 45.1880038403
    
    ##These values are pre-calculated and needs to be extracted from a specific location 
     
    cp3_p_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\load_only_storage_milp_master_level_models\\look_up_tables\\'      ##Distribution pump look up table location 
    cp3_p_table_filename = 'evap_cond_pump_lincoeff.csv'
    cp3_p_table = pd.read_csv(cp3_p_look_up_table_loc + cp3_p_table_filename)
    
    cp3_p_coeff = cp3_p_table['p_coeff'][5]
    cp3_m_coeff = cp3_p_table['m_coeff'][5]
    cp3_cst = cp3_p_table['cst'][5]
    cp3_max_m = cp3_p_table['max_flow'][5]
    cp3_maxpressureatmaxrpm = cp3_p_table['max_pressure'][5]
    ...
    cp3_dc[1,0] = cp3_c1
    cp3_dc[2,0] = cp3_c2
    cp3_dc[3,0] = cp3_max_m
    cp3_dc[4,0] = cp3_steps

    cp3_calc = cond_pump3_compute(cp3_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(cp3_steps)):
        ud = {}
        ud['Name'] = 'cp3_' + str(i + 1)
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = 'delp'                                                                  
        ud['Fmin_v1'] = cp3_calc['lb'][i] * cp3_max_m
        ud['Fmax_v1'] = cp3_calc['ub'][i] * cp3_max_m                                                                              
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = cp3_maxpressureatmaxrpm                                                                                  
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ...
        ud['Fmax'] = 0
        ud['Cost_v1_2'] = 0
        ud['Cost_v1_1'] = cp3_time_dependent_elect_tariff * cp3_m_coeff
        ud['Cost_v2_2'] = 0
        ud['Cost_v2_1'] = cp3_time_dependent_elect_tariff * cp3_p_coeff
        ud['Cost_v1_v2'] = 0
        ud['Cost_cst'] = cp3_time_dependent_elect_tariff * cp3_cst
        ud['Cinv_v1_2'] = 0
        ud['Cinv_v1_1'] = 0
        ud['Cinv_v2_2'] = 0
        ud['Cinv_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = cp3_m_coeff
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = cp3_p_coeff
        ud['Power_v1_v2'] = 0
        ud['Power_cst'] = cp3_cst
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(cp3_steps)):
        ...
        ##Stream --- flowrate into the chiller 3 condenser pump from chiller 3  
        stream = {}                         
        stream['Parent'] = 'cp3_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'cp3_' + str(i + 1) + '_flow_in'
        ...
        ##Stream --- pressure from chiller 3 and condenser network to chiller 3 condenser pump 
        stream = {}                                                                
        stream['Parent'] = 'cp3_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'cp3_' + str(i + 1) + '_delp_in'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(cp3_steps)):
        eqn = {}
        eqn['Name'] = 'cp3_' + str(i + 1) + '_max_pressure'
        eqn['Type'] = 'stream_limit_modified'                                      
        eqn['Sign'] = 'less_than_equal_to'                                    
        eqn['RHS_value'] = cp3_calc['int'][i]
    ...
    for i in range (0, int(cp3_steps)):   
        term = {}
        term['Parent_unit'] = 'cp3_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_cp3'
        ...
        term = {}
        term['Parent_unit'] = 'cp3_' + str(i + 1)
        term['Parent_eqn'] = 'cp3_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = -cp3_calc['grad'][i]
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 1
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = 0
