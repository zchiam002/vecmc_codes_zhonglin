...
def dist_nwk_pump (dist_np_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    ...
    dist_np_nwk_choice = dist_np_mdv['Value'][0]
    dist_np_steps = dist_np_mdv['Value'][1]

    ##Defined constants
    
    ##The constants are chosen based on predefined pumps combinations
     
    dist_np_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\manu_revision_chiller_only_with_dist_nwk_master\\look_up_tables\\'      ##Distribution pump look up table location 
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(dim_ret_values_pump[0])):
        ...
        stream['Name'] = 'dist_np_combi_' + str(ret_values_pump['combi'][i]) + '_' + str(ret_values_pump['step'][i]) + '_flow_out'
