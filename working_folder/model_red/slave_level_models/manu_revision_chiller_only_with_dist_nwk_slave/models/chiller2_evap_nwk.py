## This is a chiller2 evaporator network model, formulated as an input to a quadratic program
...
def chiller2_evap_nwk (ch2_enwk_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    ...
    ##ch2_enwk_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch2_enwk_steps = ch2_enwk_mdv['Value'][0]
    ch2_enwk_tf = ch2_enwk_mdv['Value'][1]
    
    ##Defined constants 
    ch2_enwk_coeff = 3.07492E-05
    ch2_enwk_com_coeff = 1.66667E-05
    ch2_enwk_max_flow = 1000                    ##This upper bound is needed for the computation of stepwise pressure
    ...
    ch2_enwk_dc[0,0] = ch2_enwk_tf
    ch2_enwk_dc[1,0] = ch2_enwk_coeff
    ch2_enwk_dc[2,0] = ch2_enwk_com_coeff
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch2_enwk_steps)):
        ...
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = 0
        stream['Stream_coeff_cst'] = ch2_enwk_calc['int'][i]
        stream['InOut'] = 'out'
