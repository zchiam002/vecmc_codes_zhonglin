...
def ga_scsd_run_ea (variables1, iteration):
    ...
    ##Extra step
    variables = [variables1[0], variables1[1]]
    ##Current hour
    current_hour = 11

    ##Location for the demand data 
    demand_file_loc = 'C:\\Optimization_zlc\\input_data\\conference_chiller_only_input_data\\high_load\\high_demand_' + str(current_hour) + '.csv'
    ...
    weather_file_loc = 'C:\\Optimization_zlc\\input_data\\conference_chiller_only_input_data\\high_load\\high_demand_weather.csv'
    weather_file_pd = pd.read_csv(weather_file_loc)
    
    tcond_in_ch = 273.15 + 5 + weather_file_pd['T_WB'][current_hour]
    
    ##Manually determining the max delt of the combined substation 
    max_dt_file_loc = 'C:\\Optimization_zlc\\input_data\\conference_chiller_only_input_data\\high_load\\high_demand_cbss_maxdt.csv'
    max_dt_file_pd = pd.read_csv(max_dt_file_loc)
    
    max_dt_cbss = max_dt_file_pd['Req delt'][current_hour]
    ...
    for i in range (0, num_time_steps):    
        ...
        var_input_cts = slave_var_input_curr_ts (i, variables, var_per_ts, tcond_in_ch, max_dt_cbss)
        ...
        sys.path.append('C:\\Optimization_zlc\\slave_level_models\\conference_chiller_only_slave_convex\\')
...
def convert_mdv_to_slave_param (thread_number, var, demand,  piecewise_steps):
    ...
    import sys 
    sys.path.append('C:\\Optimization_zlc\\master_level_models\\conference_chiller_only_master_level_models\\prep_functions\\')
    import os.path
...
def slave_var_input_curr_ts (current_ts, total_var, num_var, tcond_in_ch, max_dt_cbss):
    ...
    var_curr_ts.append(max_dt_cbss)
