...
def chiller2_compute (ch2_dc):
    ...
    for i in range (0, int(ch2_dc[6,0])):
        
        ##Determining the upper and lower bounds of the capacity 
        if i == 0:
            qe_lb = 0.0001
            qe_ub = (1 / ch2_dc[6,0]) * ch2_dc[1,0]
        else:
            qe_lb = i * (1 / ch2_dc[6,0]) * ch2_dc[1,0]
            qe_ub = (i + 1) * (1 / ch2_dc[6,0]) * ch2_dc[1,0]
            
        lb_frac = i * (1/ch2_dc[6,0])
        ub_frac = (i + 1) * (1 / ch2_dc[6,0])
        
        ##Computing the return temperature of the chiller on the evaporator side
        ret_temp_chiller = ch2_dc[7,0] + ch2_dc[8,0]
        ##Computing the inlet temperature of the chiller on the condenser side 
        tcond_in_chiller = ch2_dc[0,0] + 273.15 + 5
        
        cop1, qc1 = gnu_chiller2_4nc (ch2_dc[2,0], ch2_dc[3,0], ch2_dc[4,0], ch2_dc[5,0], ret_temp_chiller, tcond_in_chiller, qe_lb)
        cop2, qc2 = gnu_chiller2_4nc (ch2_dc[2,0], ch2_dc[3,0], ch2_dc[4,0], ch2_dc[5,0], ret_temp_chiller, tcond_in_chiller, qe_ub)
    ...
    enwk_max_flow = (3600 * ch2_dc[1,0]) / (ch2_dc[11,0] * ch2_dc[8,0] * 998.2)
    
    ##Determining the step size 
    enwk_step_size = 1 / ch2_dc[6,0]
    ...
    for i in range (0, int(ch2_dc[6,0])):
        ...
        enwk_delp_lb = (ch2_dc[10,0] * pow(enwk_flow_lb, 1.852))
        enwk_delp_ub = (ch2_dc[10,0] * pow(enwk_flow_ub, 1.852))
    ...
    ##Determining the maximum flowrate for the evaporator network (excluding the shared area)
    cnwk_e_at_max_qe = (ch2_dc[1,0] * ret_vals['grad'][ch2_dc[6,0] - 1]) + ret_vals['int'][ch2_dc[6,0] - 1]
    cnwk_max_qc = ch2_dc[5,0] * (cnwk_e_at_max_qe + ch2_dc[1,0])
    cnwk_max_flow = (3600 * cnwk_max_qc) / (ch2_dc[11,0] * ch2_dc[9,0] * 998.2)    
    
    ##Determining the step size 
    cnwk_step_size = 1 / ch2_dc[6,0]
    ...
    for i in range (0, int(ch2_dc[6,0])):
        ...
        cnwk_delp_lb = (ch2_dc[12,0] * pow(cnwk_flow_lb, 1.852)) 
        cnwk_delp_ub = (ch2_dc[12,0] * pow(cnwk_flow_ub, 1.852))
...
def gnu_chiller2_4nc (b0, b1, b2, qc_coeff, Tin_evap, Tin_cond, Qe):
