...
def checktype_chiller3_cond_pump_4nc (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller3_cond_pump_4nc (ch3_cp_4nc_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from chiller3_cond_pump_4nc_compute import chiller3_cond_pump_4nc_compute
    import pandas as pd
    import numpy as np
    ...
    ##ch3_cp_4nc_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch3_cp_steps = ch3_cp_4nc_mdv['Value'][0]

    ##Defined constants 
    ch3_cp_c0 = -0.0000090818       ##These are pump cure coefficients which are needed to formulate constraints                   
    ch3_cp_c1 = 0.0029568794
    ch3_cp_c2 = 45.1880038403
    
    ##These values are pre-calculated and needs to be extracted from a specific location 
     
    ch3_cp_p_look_up_table_loc = 'C:\\Optimization_zlc\\master_level_models\\paper_qe_testcase_master_level_models\\look_up_tables\\'      ##Distribution pump look up table location 
    ch3_cp_p_table_filename = 'evap_cond_pump_lincoeff.csv'
    ch3_cp_p_table = pd.read_csv(ch3_cp_p_look_up_table_loc + ch3_cp_p_table_filename)
    
    ch3_cp_p_coeff = ch3_cp_p_table['p_coeff'][5]
    ch3_cp_m_coeff = ch3_cp_p_table['m_coeff'][5]
    ch3_cp_cst = ch3_cp_p_table['cst'][5]
    ch3_cp_max_m = ch3_cp_p_table['max_flow'][5]
    ch3_cp_maxpressureatmaxrpm = ch3_cp_p_table['max_pressure'][5]
    
    ##Calling a compute file to process dependent values
        ##Placing the values in a numpy array for easy data handling 
    ch3_cp_dc = np.zeros((5,1))
    
    ch3_cp_dc[0,0] = ch3_cp_c0
    ch3_cp_dc[1,0] = ch3_cp_c1
    ch3_cp_dc[2,0] = ch3_cp_c2
    ch3_cp_dc[3,0] = ch3_cp_max_m
    ch3_cp_dc[4,0] = ch3_cp_steps

    ch3_cp_calc = chiller3_cond_pump_4nc_compute(ch3_cp_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch3_cp_steps)):
        ud = {}
        ud['Name'] = 'ch3_cp_4nc_' + str(i + 1)
        ud['Variable1'] = 'm_flow'                                                                        
        ud['Variable2'] = 'delp'                                                                  
        ud['Fmin_v1'] = ch3_cp_calc['lb'][i] * ch3_cp_max_m
        ud['Fmax_v1'] = ch3_cp_calc['ub'][i] * ch3_cp_max_m                                                                              
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = ch3_cp_maxpressureatmaxrpm                                                                                  
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
        ud['Coeff_v2_2'] = 0
        ud['Coeff_v2_1'] = 0
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = ch3_cp_m_coeff
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = ch3_cp_p_coeff
        ud['Power_v1_v2'] = 0
        ud['Power_cst'] = ch3_cp_cst
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch3_cp_steps)):
        ...
        ##Stream --- flowrate into the chiller 3 condenser pump from chiller 3 condenser network 
        stream = {}                         
        stream['Parent'] = 'ch3_cp_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch3_cp_4nc_' + str(i + 1) + '_flow_in'
        stream['Layer'] = 'ch3condnwk_2_ch3condpump_flow' 
        ...
        ##Stream --- pressure from chiller 3 condenser network to chiller 3 condenser pump
        stream = {}                                                                
        stream['Parent'] = 'ch3_cp_4nc_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'ch3_cp_4nc_' + str(i + 1) + '_delp_in'
        stream['Layer'] = 'ch3condnwk_2_ch3condpump_delp'
        stream['Stream_coeff_v1_2'] = 0
        ...
        ##Stream --- flowrate from chiller 3 condenser pump to distribution network
        stream = {}                                                                
        stream['Parent'] = 'ch3_cp_4nc_' + str(i + 1)
        stream['Type'] = 'flow_reverse'
        stream['Name'] = 'ch3_cp_4nc_' + str(i + 1) + '_flow_out'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch3_cp_steps)):
        eqn = {}
        eqn['Name'] = 'ch3_cp_4nc_' + str(i + 1) + '_max_pressure'
        eqn['Type'] = 'stream_limit_modified'                                      
        eqn['Sign'] = 'less_than_equal_to'                                    
        eqn['RHS_value'] = ch3_cp_calc['int'][i]
    ...
    for i in range (0, int(ch3_cp_steps)):   
        term = {}
        term['Parent_unit'] = 'ch3_cp_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ch3_cp_4nc'
        ...
        term = {}
        term['Parent_unit'] = 'ch3_cp_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'ch3_cp_4nc_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
        term['Coeff_v1_1'] = -ch3_cp_calc['grad'][i]
        term['Coeff_v2_2'] = 0
        term['Coeff_v2_1'] = 1
        term['Coeff_v1_v2'] = 0
        term['Coeff_cst'] = 0
