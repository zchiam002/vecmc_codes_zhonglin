...
def layers_nwk_choice_4 (layerslist):
    ...
    layer = ['temp_chil', 'sp1_temp2cp']                                ##Layer to handle all chiller evaporator outlet temperatures
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)   
    
    layer = ['temp_chil', 'sp1_temp2gv2']                                ##Layer to handle all chiller evaporator outlet temperatures
    layerdf = pd.DataFrame(data = [layer], columns=['Type', 'Name'])
    layerslist = layerslist.append(layerdf, ignore_index=True)
