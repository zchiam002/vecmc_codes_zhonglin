## This is a ser network model, formulated as an input to a quadratic program
...
def checktype_ser_nwk_4nc (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def ser_nwk_4nc (ser_nwk_4nc_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    ...
    ##ser_nwk_4nc_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ser_nwk_steps = ser_nwk_4nc_mdv['Value'][0]
    ser_nwk_tf = ser_nwk_4nc_mdv['Value'][1]
    ser_nwk_max_flow = ser_nwk_4nc_mdv['Value'][1] *0.35                ##This upper bound is needed for the computation of stepwise pressure
                                                                        ##The estimated proportion of cooling consumed by ser 
    ##Defined constants 
    ser_nwk_coeff = 0.000536956604603528
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ser_nwk_steps)):
        ud = {}
        ud['Name'] = 'ser_nwk_4nc_' + str(i + 1)
        ud['Variable1'] = 'm_perc'                                                                        
        ud['Variable2'] = '-'                                                                  
        ud['Fmin_v1'] = ser_nwk_calc['lb'][i]
        ud['Fmax_v1'] = ser_nwk_calc['ub'][i]                                                                                
        ud['Fmin_v2'] = 0                                                                                      
        ud['Fmax_v2'] = 0                                                                                   
        ud['Coeff_v1_2'] = 0                                                                                    
        ud['Coeff_v1_1'] = 0 
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ser_nwk_steps)):
        ...
        ##Stream --- flowrate into the ser network from tro network
        stream = {}                         
        stream['Parent'] = 'ser_nwk_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ser_nwk_4nc_' + str(i + 1) + '_flow_in'
        stream['Layer'] = 'tro_outlet_flow'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ser_nwk_tf
        ...
        ##Stream --- flowrate from ser network into ser substation
        stream = {}                                                                
        stream['Parent'] = 'ser_nwk_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ser_nwk_4nc_' + str(i + 1) + '_flow_out'
        ...
        ##Stream --- pressure from ser network into ser pressure layer
        stream = {}                                                                
        stream['Parent'] = 'ser_nwk_4nc_' + str(i + 1)
        stream['Type'] = 'pressure'
        stream['Name'] = 'ser_nwk_4nc_' + str(i + 1) + '_delp_out'
        stream['Layer'] = 'ser_consol_delp'
        stream['Stream_coeff_v1_2'] = 0
        stream['Stream_coeff_v1_1'] = ser_nwk_tf  * ser_nwk_calc['grad'][i]
        stream['Stream_coeff_v2_2'] = 0
        stream['Stream_coeff_v2_1'] = 0
        stream['Stream_coeff_v1_v2'] = 0
        stream['Stream_coeff_cst'] = ser_nwk_calc['int'][i]
        stream['InOut'] = 'out'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ser_nwk_steps)):   
        term = {}
        term['Parent_unit'] = 'ser_nwk_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ser_nwk_4nc'
