...
def checktype_chiller2_evap_pump_4nc (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller2_evap_pump_4nc (ch2_ep_4nc_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from chiller2_evap_pump_4nc_compute import chiller2_evap_pump_4nc_compute
    import pandas as pd
    import numpy as np
    import os
    ...
    ##ch2_ep_4nc_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch2_ep_steps = ch2_ep_4nc_mdv['Value'][0]
    ...
    ch2_ep_p_look_up_table_loc = current_directory + 'milp_model\\master_level_models\\chiller_optimization_dist_nwk_a2c_master_level_models\\look_up_tables\\'      ##Evaporator pump look up table location 
    ...
    ch2_ep_calc = chiller2_evap_pump_4nc_compute(ch2_ep_dc)
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch2_ep_steps)):
        ...
        stream = {}                         
        stream['Parent'] = 'ch2_ep_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch2_ep_4nc' + str(i + 1) + '_flow_in'
        ...
        stream['Name'] = 'ch2_ep_4nc' + str(i + 1) + '_delp_in'
        ...
        stream['Parent'] = 'ch2_ep_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch2_ep_4nc' + str(i + 1) + '_flow_out'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch2_ep_steps)):
        eqn = {}
        eqn['Name'] = 'ch2_ep_4nc_' + str(i + 1) + '_max_pressure'
    ...
    for i in range (0, int(ch2_ep_steps)):   
        term = {}
        term['Parent_unit'] = 'ch2_ep_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ch2_ep_4nc'
        ...
        term = {}
        term['Parent_unit'] = 'ch2_ep_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'ch2_ep_4nc_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
