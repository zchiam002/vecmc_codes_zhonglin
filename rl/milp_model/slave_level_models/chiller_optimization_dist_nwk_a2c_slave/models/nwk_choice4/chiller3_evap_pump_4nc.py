...
def checktype_chiller3_evap_pump_4nc (unit_type):                  ##Input the unit type here
    unit_type = 'utility'
    return unit_type

def chiller3_evap_pump_4nc (ch3_ep_4nc_mdv, utilitylist, streams, cons_eqns, cons_eqns_terms):
    
    from chiller3_evap_pump_4nc_compute import chiller3_evap_pump_4nc_compute
    import pandas as pd
    import numpy as np
    import os
    current_directory = os.path.dirname(__file__)[:-90] + '//' 
    
    ##Model description: 
    ##Built on pressure drop ratios based on a fixed flowrate to estimate the electricity cost  
    
    ##ch3_ep_4nc_mdv --- the master decision variables which are used as parameters at this stage 
    ...
    ch3_ep_steps = ch3_ep_4nc_mdv['Value'][0]
    ...
    ch3_ep_p_look_up_table_loc = current_directory + 'milp_model\\master_level_models\\\chiller_optimization_dist_nwk_a2c_master_level_models\\look_up_tables\\'      ##Evaporator pump look up table location 
    ...
    ch3_ep_calc = chiller3_evap_pump_4nc_compute(ch3_ep_dc)
...
#################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch3_ep_steps)):
        ...
        ud['Cinv_cst'] = 0
        ud['Power_v1_2'] = 0
        ud['Power_v1_1'] = ch3_ep_m_coeff
        ud['Power_v2_2'] = 0
        ud['Power_v2_1'] = ch3_ep_p_coeff
        ud['Power_v1_v2'] = 0
        ud['Power_cst'] = ch3_ep_cst
        ud['Impact_v1_2'] = 0
        ud['Impact_v1_1'] = 0
        ud['Impact_v2_2'] = 0
        ud['Impact_v2_1'] = 0
...
####################################################################################################################################################################################################        
    ...
    for i in range (0, int(ch3_ep_steps)):
        ...
        stream = {}                         
        stream['Parent'] = 'ch3_ep_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch3_ep_4nc_' + str(i + 1) + '_flow_in'
        ...
        stream['Name'] = 'ch3_ep_4nc_' + str(i + 1) + '_delp_in'
        ...
        stream['Parent'] = 'ch3_ep_4nc_' + str(i + 1)
        stream['Type'] = 'flow'
        stream['Name'] = 'ch3_ep_4nc_' + str(i + 1) + '_flow_out'
...
############################################################################################################################################################################################################
    ...
    for i in range (0, int(ch3_ep_steps)):
        eqn = {}
        eqn['Name'] = 'ch3_ep_4nc_' + str(i + 1) + '_max_pressure'
    ...
    for i in range (0, int(ch3_ep_steps)):   
        term = {}
        term['Parent_unit'] = 'ch3_ep_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'totaluse_ch3_ep_4nc'
        ...
        term = {}
        term['Parent_unit'] = 'ch3_ep_4nc_' + str(i + 1)
        term['Parent_eqn'] = 'ch3_ep_4nc_' + str(i + 1) + '_max_pressure'
        term['Parent_stream'] = '-'                                    ##Only applicable for stream_limit types 
        term['Coefficient'] = 0
        term['Coeff_v1_2'] = 0                                         ##Only applicable for stream_limit_modified types 
