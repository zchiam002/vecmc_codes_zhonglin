...
def a2c_resume_training_saved_model (env, action_tf_var, action_placeholder, training_op_actor, loss_actor, state_placeholder, last_layer, loss_critic, training_op_critic, 
                                     training_episodes, target_placeholder, delta_placeholder, save_directory, exploration_epsilon, critic_input_placeholder, save_file_name, 
    ...
    import sys
    sys.path.append(current_directory + 'master_level\\reinforcement_learning_algorithm\\actor_critic_advantage_milp\\')
    from preprocessing_module import scale_state
    from a2c_backup_before_resume import a2c_backup_before_resume
    import pandas as pd
    ...
    proceed = a2c_backup_before_resume (save_file_name, resume_training_count, save_folder_directory)
...
#                critic_input = np.array([critic_input])
                ...
                predicted_values = sess.run(last_layer, feed_dict = {critic_input_placeholder: state}) 
...
#                    critic_input1 = np.array([critic_input1])
                    ...
                    ##Output from the second last layer of the critic network 
                    predicted_values = sess.run(last_layer, feed_dict = {critic_input_placeholder: state1}) 
...
#                    critic_input = np.concatenate((critic_input, critic_input1), axis = 0)
                ...
                _, loss_critic_val  = sess.run([training_op_critic, loss_critic], feed_dict={critic_input_placeholder: state, target_placeholder: env_output_values})   
                ...
                if i%100 == 0:
                    ...
                    folder_save_dir = save_dir[:-len('a2c_trained.ckpt')]
